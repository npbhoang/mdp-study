...
def personalized_stats(request):
    return render_template('personalized_stats.html', user=current_user, ad=get_personalize_ad({'user': current_user}))
...
def send_invite(request):
    ...
    invite.invitee = user
    db.session.commit()
    return render_template('manage_event.html', event=Event.query.get(request.args["e"]))
