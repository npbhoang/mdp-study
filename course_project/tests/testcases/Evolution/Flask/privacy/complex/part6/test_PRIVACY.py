...
# Should pass
def test_case_235_FUNCTIONAL_remove_attendee(client, app_with_data):
...
# on endpoint remove_attendee() 
# Should pass
def test_case_234_ANY_remove_attendee(client, app_with_data):
    caller = Person.query.filter(Person.testid == 'ADMIN0').one()
    owner = Person.query.get(5)
    with app_with_data.test_request_context():
        login_user(caller)
...
# Should pass
def test_case_242_ANY_accept_request(client, app_with_data):
    caller = Person.query.filter(Person.testid == 'ADMIN0').one()
