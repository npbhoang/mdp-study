...
from dtm import db, Event, Category, Person, Role, Ad, Purpose, Consent, PersonalData, Invite
...
def user(id):
    ...
    user_subscriptions = user.subscriptions.__len__()
...
def profile():
    ...
    manage_event_titles = [e.title for e in manage_events]
    manage_event_owners = [e.owner for e in manage_events]
    manage_event_owner_names = [o.name for o in manage_event_owners]
    attend_events = [e for e in current_user.attends]
    attend_event_titles = [e.title for e in attend_events]
    attend_event_owners = [e.owner for e in attend_events]
    attend_event_owner_names = [o.name for o in attend_event_owners]
    subscriptions = current_user.subscriptions.__len__()
    ...
    for c in current_user.subscriptions:
        for e in c.events:
...
def view_event(id):
    ...
    event_category_names = [c.name for c in event_categories]
    event_attendants = event.attendants
    event_attendant_names = [a.name for a in event_attendants]
    event_attendant_surames = [a.surname for a in event_attendants]
    return {
        'event': Event.query.get(id),
...
def manage_event(id):
    ...
    event_attendants = event.attendants
    event_attendant_names = [a.name for a in event_attendants]
    event_attendant_surnames = [a.surname for a in event_attendants]
    event_managers = event.managedBy
    event_manager_names = [m.name for m in event_managers]
    event_manager_surnames = [m.surname for m in event_managers]
    event_requesters = event.requesters
    event_requester_names = [r.name for r in event_requesters]
    event_requester_surnames = [r.surname for r in event_requesters]
    return {
        'event': Event.query.get(id),
        'event_attendants': event_attendants,
        'event_attendant_names': event_attendant_names,
...
def view_category(id):
    category = Category.query.get(id)
    category_subscribers = category.subscribers
    category_subscriber_names = [s.name for s in category_subscribers]
    category_subscriber_surnames = [s.surname for s in category_subscribers]
    category_events = category.events
    category_event_titles = [e.title for e in category_events]
    category_event_owners = [e.owner for e in category_events]
...
def edit_category(id):
    ...
    category_moderator_names = [m.name for m in category_moderators]
    category_moderator_surnames = [m.surname for m in category_moderators]
...
def personalized_stats(request):
    managed_events = [e for e in current_user.manages]
    attended_events = [e for e in current_user.attends]
    subscriptions = [c for c in current_user.subscriptions]
    invitations = [i for i in current_user.invitations]
    return {
